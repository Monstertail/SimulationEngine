  ...
  val liftedMain = meta.classLifting.liteLift {
    def apply(width: Int, height: Int, col:Int, row:Int): IndexedSeq[Actor] = {
      ...
      val tiles = Range(0, totalComp).map(i => {
        val L2DA = new example.gameOfLifeMultiTileTopoMsgF.LocalArray2D[Boolean](width, height, new example.gameOfLifeMultiTileTopoMsgF.gridCoordinate((i / col)*height, (i % col)*width))
