...
//  var newBoard: Array[Array[V]] = Array.ofDim[V](h, w)
  ...
  var IncMsgBuff=Array.ofDim[ArrayBuffer[V]](h*w)
  ...
  private def actionPerCell(cell_state:Boolean, msgBuffer:Iterable[Boolean]):Boolean = {

        val aliveNeighbors = msgBuffer.count(_ == true)
        if (cell_state) {
...
//              }
        ...
        IncMsgBuff(i*width+j) = neighbors.asInstanceOf[ArrayBuffer[V]]
   ...
   for (i <- 0 until height) {
     ...
     for (j <- 0 until width) {
       ...
       currentBoard(i)(j) = actionPerCell(currentBoard(i)(j).asInstanceOf[Boolean], IncMsgBuff(i*width+j).asInstanceOf[Iterable[Boolean]]).asInstanceOf[V]
     }
   }
