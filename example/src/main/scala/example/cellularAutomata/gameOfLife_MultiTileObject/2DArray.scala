package example
package gameOfLifeMultiTileObject
...
class Array2D(val width: Int, val height: Int) {
  ...
  var tileNO: Int = _
  var currentBoard: Array[Array[Cell]] = Array.ofDim[Cell](height, width)
  var newBoard: Array[Array[Cell]] = Array.ofDim[Cell](height, width)
  ...
  def init(tN:Int): Unit = {
    ...
    for (i <- 0 until height; j <- 0 until width) {
      currentBoard(i)(j)=new Cell(0)
      newBoard(i)(j)=new Cell(0)
      currentBoard(i)(j).state = random.nextBoolean()
    }
    tileNO=tN
  }

  def packageMsg(): Vector[Vector[Boolean]] = {
    val topEdge: Vector[Boolean] = currentBoard(0).map(_.state).toVector
    val bottomEdge:Vector[Boolean] = currentBoard(height-1).map(_.state)toVector
    val leftEdge: Vector[Boolean] = currentBoard.map(row => row(0)).map(_.state).toVector
    val rightEdge: Vector[Boolean] = currentBoard.map(row => row(row.length-1)).map(_.state).toVector
    val topLeft:Vector[Boolean] = Vector(currentBoard(0)(0).state)
    val topRight:Vector[Boolean] = Vector(currentBoard(0)(width-1).state)
    val bottomLeft:Vector[Boolean] = Vector(currentBoard(height-1)(0).state)
    val bottomRight:Vector[Boolean] = Vector(currentBoard(height-1)(width-1).state)
...
//              if (currentBoard(row)(col)) aliveNeighbors = aliveNeighbors + 1
 ...
 def printBooleanArray(arr: Array[Array[Cell]]): Unit = {
   println(s"tile id: $tileNO")
    for ( m <- arr.indices) {
      for (n <- arr(m).indices) {
        if (arr(m)(n).state) print(1)
        else print(0)
      }
      println()
    }
